...
- **Cross-region inference profiles (CRIS)** are NOT separate Pricing SKUs. Probing `usagetype=USE1-us.anthropic.claude-sonnet-4-6-input-tokens` returns 0 results. CRIS calls are billed at the source region's rate using the bare-modelId SKU. The meter strips the regional prefix (`us.`/`eu.`/`apac.`/`ap.`) before pricing lookup.
