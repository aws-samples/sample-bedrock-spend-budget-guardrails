...
- 🩺 **Pre-deployment readiness audit** — a one-click **Readiness** page (admin) answers "is this account ready for per-principal Bedrock cost attribution?" *before* you roll BBG out. An async audit inventories every Bedrock-capable IAM principal, their CloudTrail Bedrock activity, application inference profiles, 90-day Cost Explorer spend, and cost-allocation tag coverage (team / cost-center / environment / project), then returns an explainable **GREEN / YELLOW / RED** Tier-1 verdict with per-account reasoning, recommendations, downloadable HTML + CSV reports, and a generated `setup-tier1.sh` remediation script. Runs against the home account, or from an Org management account it auto-pivots across every member account (read-only via `OrganizationAccountAccessRole`, parallelized). Lives in [`lambda/python/readiness/`](lambda/python/readiness/).
- 🌙 **Cloudscape web app** — dark-mode-first React + Vite (single-page app) with sortable / filterable Pricing, Inference Profiles, Identities, Budgets, Reports (Athena), Readiness, Enrollment, Audit log, and Spend pages. Trend charts, top-spender bars, per-model dimension breakdowns, per-region and per-account spend charts.
